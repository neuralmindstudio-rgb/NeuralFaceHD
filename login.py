...
class TelaLogin(Screen):
    ...
    def resetar_senha(self, instance):
        ...
        if self.input_email.text:
            try:
                ...
                MDDialog(title="Erro", text=f            mode="rectangle",
            size_hint_y=None,
            height=dp(56)
        )
        ...
        if self.input_email.text:
            try:
                ...
                if ok:
                    ...
                    erro_real = getattr(bd, "ultimo_erro", "ERRO_DESCON        self.input_email.bind(on_touch_down=self.forcar_foco)
        self.input_email.bind(on_touch_up=self.forcar_foco_up)
    ...
    def resetar_senha(self, instance):
        ...
        if self.input_email.text:
            ...
            MDDialog(title="Aviso", text="Digite o e-mail.").open()            Color(0, 0, 0, 1)
            self.rect = Rectangle(pos=self.pos, size=self.size)
        self.bind(pos=self.update_rect, size=self.update_rect)
